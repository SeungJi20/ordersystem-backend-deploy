package com.example.order_system.common.service;

import com.example.order_system.common.dto.SseMessageDto;
import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import lombok.RequiredArgsConstructor;
import org.springframework.data.redis.connection.Message;    // ✅ Redis용
...
public class SseAlarmService implements MessageListener {
    ...
    // 특정 사용자에게 message 발송                              <--어떤걸 넘겨줄지 정하기(커스텀)-->
    public void publishMessage(String receiver, String sender, Long orderingId){
        ...
        ObjectMapper objectMapper = new ObjectMapper(); // 조립
        String data = null;

        try {
           data = objectMapper.writeValueAsString(dto); // JSON으로
        ...
        // 사용자가 로그아웃(새로고침)후에 다시 화면에 들어왔을 때 알림메시지가 남아있으려면 DB에 추가적으로 저장 필수
            } catch (IOException e) {
                e.printStackTrace();
            }
        }else{
