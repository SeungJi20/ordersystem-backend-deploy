...
public class JwtTokenProvider {
    ...
    @Value("${jwt.expirationAt}") // yml 파일에서 선언했던 값이 들어옴
    private int expirationAt;

    @Value("${jwt.secretKeyAt}")
    private String secretKeyAt;

    @Value("${jwt.expirationRt}") // yml 파일에서 선언했던 값이 들어옴
    ...
    private Key secret_rt_key; // Key라는 객체로 감싸서 decoding한 것

    // Qualifier는 기본적으로 메서드를 통한 주입이 가능. 그래서, 이 경우 생성자 주입방식을 해야 Qualifier사용 가능.
    ...
    public String createAtToken(Member member) {
        ...
        // 주된 키값을 제외한 나머지 사용자정보는 put 사용해서 key:value 세팅
    ...
    public String createRtToken(Member member) {
        ...
        String refreshToken = Jwts.builder()
                .setClaims(claims)
                .setIssuedAt(now)
                .setExpiration(new Date(now.getTime() + expirationRt*60*1000L)) // 30분을 세팅(밀리초)
                // secret 키를 통해 signiture 생성
